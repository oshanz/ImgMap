...
define(function(require) {
    ...
    $.fn.ExportExcel = function(title, rc_array) {
        if (this.prop("nodeName") == "TABLE") {
            var dom = this.clone().get(0);
    ...
    $.fn.print = function() {
        if (1 < this.size())
            ...
            var b = "printer-" + (new Date).getTime(), c = $("<iframe name='" + b + "'>");
            c.css("width", "1px").css("height", "1px").css("position", "absolute").css("left", "-9999px").appendTo($("body:first"));
            var b = window.frames[b], a = b.document, d = $("<div>").append($("style").clone());
            a.open();
            a.write('<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">');
            a.write("<html>");
            a.write("<body>");
            a.write("<head>");
            a.write("<title>");
            a.write(document.title);
            a.write("</title>");
            a.write(d.html());
            a.write("</head>");
            a.write(this.html());
    ...
    ob.number_format = function(b, c, d, e) {
